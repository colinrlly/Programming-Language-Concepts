...
Value eval(Exp e, Env env) {
    Value v;
    Frame *fr;
    /* definition of static [[Exp hole]], which always has a hole 1076c */
    static struct Exp holeExp = { HOLE, { { NIL, { 0 } } } };
    static Exp hole = &holeExp;
    static Stack evalstack;

    /* ensure that [[evalstack]] is initialized and empty 1070b */
    if (evalstack == NULL)
        evalstack = emptystack();
    else
        clearstack(evalstack);
    /* ensure that [[evalstack]] is initialized and empty 1092b */
    assert(topframe(roots.stack) == NULL);
    roots.stack = evalstack;
    /* use the options in [[env]] to initialize the instrumentation 1072d */
    high_stack_mark = 0;
    show_high_stack_mark = 
        istrue(getoption(strtoname("&show-high-stack-mark"), env, falsev));
    /* use the options in [[env]] to initialize the instrumentation 1073b */
    ...
    /* use the options in [[env]] to initialize the instrumentation 1075f */
    optimize_tail_calls = 
        istrue(getoption(strtoname("&optimize-tail-calls"), env, truev));

    exp: 
        stack_trace_current_expression(e, env, evalstack);
        /* take a step from a state of the form $\seval e$ 271 */
        switch (e->alt) {
        case LITERAL:

/* start evaluating expression [[e->u.literal]] and transition to the next state 273a */
            v = e->u.literal;
            goto value;
        case VAR:   

/* start evaluating expression [[e->u.var]] and transition to the next state 273b */
...
/* start evaluating expression [[e->u.set]] and transition to the next state 274a */
...
/* start evaluating expression [[e->u.ifx]] and transition to the next state 274c */
...
/* start evaluating expression [[e->u.whilex]] and transition to the next state 282d */
...
/* start evaluating expression [[e->u.begin]] and transition to the next state 282b */
            pushcontext(mkBeginStruct(e->u.begin), evalstack);
            v = falsev;
            goto value;
        case LETX:
            if (/* [[e->u.letx]] contains no bindings 278b */
...
/* start evaluating nonempty [[let]] expression [[e->u.letx]] and transition to the next state 278c */
...
/* start evaluating nonempty [[let*]] expression [[e->u.letx]] and transition to the next state 279b */
...
/* start evaluating nonempty [[letrec]] expression [[e->u.letx]] and transition to the next state 279a */
                      pushenv_opt(env, LETXENV, evalstack);
                      {   Namelist xs;
...
/* start evaluating expression [[e->u.lambdax]] and transition to the next state 273c */
            v = mkClosure(e->u.lambdax, env);
            goto value;
        case APPLY:

/* start evaluating expression [[e->u.apply]] and transition to the next state 276c */
        ...
        /* start evaluating [[(break)]] and transition to the next state 284a */
            fr = topframe(evalstack);
            if (fr == NULL) 
...
/* transition to [[WHILEX]] and continue with condition in context [[fr->context.u.whilex]] 283b */
...
/* start evaluating expression [[e->u.returnx]] and transition to the next state 284c */
            pushcontext(mkReturnxStruct(hole), evalstack);
            e = e->u.returnx;
            goto exp;
        case THROW:

/* start evaluating expression [[e->u.throw]] and transition to the next state 284e */
            pushcontext(mkThrowStruct(hole), evalstack);
            e = e->u.throw;
            goto exp;
        case TRY_CATCH:

/* start evaluating expression [[e->u.try_catch]] and transition to the next state 285a */
            pushcontext(mkTryCatchStruct(e->u.try_catch.body, hole), evalstack);
            e = e->u.try_catch.handler;
            goto exp;

/* cases where the current item is an expression form that should appear only on the stack 1076b */
        case WHILE_RUNNING_BODY:
        case HOLE:
        case LETXENV:
        case CALLENV:
...
/* if [[evalstack]] is empty, return [[v]]; otherwise step from a state of the form $\sevalv {\mathtt{fr} \sconsop S}$ 270b */
        fr = topframe(evalstack);
        if (fr == NULL) {

         /* if [[show_high_stack_mark]] is set, show maximum stack size 1072e */
            if (show_high_stack_mark)
                fprintf(stderr, "High stack mark == %d\n", high_stack_mark);
            return v;
        } else {

/* take a step from a state of the form $\sevalv {\mathtt{fr} \sconsop S}$ 272 */
            switch (fr->context.alt) {
            case SET:

/* fill hole in context [[fr->context.u.set]] and transition to the next state 274b */
...
/* fill hole in context [[fr->context.u.ifx]] and transition to the next state 274d */
...
/* if [[v]] is true, continue with body in context [[fr->context.u.whilex]] 283a */
...
/* transition to [[WHILEX]] and continue with condition in context [[fr->context.u.whilex]] 283b */
 ...
 /* continue with the next expression in context [[fr->context.u.begin]] 282c */
...
/* fill hole in context [[fr->context.u.apply]] and transition to the next state 277a */
...
/* apply [[fr->context]]'s [[fn]] to its [[actuals]]; free memory; transition to next state 277b */
                    {
  ...
  /* apply [[fn.u.primitive]] to [[vs]] and transition to the next state 277c */
                              e = fr->syntax;
...
/* save [[env]], bind [[vs]] to [[fn.u.closure]]'s formals, and transition to evaluation of closure's body 278a */
                              {
            ...
            case LETX:
                switch (fr->context.u.letx.let) {
                   case LET:
                 /* continue with [[let]] context [[fr->context.u.letx]] 280a */
                   ...
                   case LETSTAR:
                /* continue with [[let*]] context [[fr->context.u.letx]] 281b */
                   ...
                   case LETREC:
              /* continue with [[letrec]] context [[fr->context.u.letx]] 280b */
                                 e = transition_explist(fr->context.u.letx.es, v
                                                                              );
...
/* store values in [[fr->context.u.letx.es]] in locations bound to [[fr->context.u.letx.xs]] 281a */
                                     {
                                         Namelist xs = fr->context.u.letx.xs;
                                         Explist  es = fr->context.u.letx.es;
                                         while (es || xs) { 
...
/* restore [[env]] from [[fr->context.u.letxenv]], pop the stack, and transition to the next state 281c */
                env = fr->context.u.letxenv;
                popframe(evalstack);
                goto value;
            case CALLENV:

/* restore [[env]] from [[fr->context.u.callenv]], pop the stack, and transition to the next state 282a */
                env = fr->context.u.callenv;
                popframe(evalstack);
                goto value;
            case TRY_CATCH:

/* if awaiting handler, install [[v]] and evaluate body, otherwise pop stack and transition to the next state 285b */
...
/* throw [[v]] to the nearest [[try-catch]] that has an installed handler ((answers)) (BUG: software can't tell where this code came from [NW3DWkps-2YYHUt-1]) */
            ...
            "Uncaught exception; evaluated (throw e) with no active try-catch");
                } else if (fr->context.alt == TRY_CATCH && fr->
                                       context.u.try_catch.handler->alt != HOLE)
                {
                    e = mkApply(fr->context.u.try_catch.handler, mkEL(mkLiteral(
                                                                     v), NULL));
                    popframe(evalstack);
                    goto exp;
                } else {
                    fr->context = mkThrowStruct(hole);
                    goto value;
                }
