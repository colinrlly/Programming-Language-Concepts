...
void printdeflist(FILE *output, va_list_box *box) {
    for (Deflist ds = va_arg(box->ap, Deflist); ds != NULL; ds = ds->tl)
        fprint(output, "%d%s", ds->hd, ds->tl ? " " : "");
}
...
void printunittestlistlist(FILE *output, va_list_box *box) {
    for (UnitTestlistlist uss = va_arg(box->ap, UnitTestlistlist); uss != NULL;
                                                                  uss = uss->tl)
        fprint(output, "%U%s", uss->hd, uss->tl ? " " : "");
}
