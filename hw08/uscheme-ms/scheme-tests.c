...
/* scheme-tests.c 1059e */
TestResult test_result(UnitTest t, Env rho) {
    switch (t->alt) {
    case CHECK_EXPECT:
        /* run [[check-expect]] test [[t]], returning [[TestResult]] 1060a */
        {   if (setjmp(testjmp)) {

/* report that evaluating [[t->u.check_expect.check]] failed with an error 1061b */
                fprint(stderr,
...
/* report that evaluating [[t->u.check_expect.expect]] failed with an error 1061c */
                fprint(stderr,
                     "Check-expect failed: expected %e to evaluate to the same "
            ...
            if (!equalpairs(check, expect)) {
                /* report failure because the values are not equal 1061a */
                fprint(stderr,
    ...
    case CHECK_ASSERT:
        /* run [[check-assert]] test [[t]], returning [[TestResult]] 1060b */
        {   if (setjmp(testjmp)) {

   /* report that evaluating [[t->u.check_assert]] failed with an error 1061e */
                fprint(stderr,
            ...
            if (v.alt == BOOLV && !v.u.boolv) {
                /* report failure because the value is false 1061d */
    ...
    case CHECK_ERROR:
        /* run [[check-error]] test [[t]], returning [[TestResult]] 1060c */
        {   if (setjmp(testjmp)) {
                return TEST_PASSED; // error occurred, so the test passed
            }
            Value check = eval(t->u.check_error,  rho);

      /* report that evaluating [[t->u.check_error]] produced [[check]] 1061f */
            fprint(stderr,
