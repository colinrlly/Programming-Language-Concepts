...
/* make [[f]] the function denoted by [[e->u.apply.name]], or call [[runerror]] 48c */
            ...
            case USERDEF:
                /* apply [[f.u.userdef]] and return the result 49b */
                {
                    ...
                    // set up a temporary environment to with the combination
                    Namelist tempEnv = NULL;

                    // set up a final environment to hold the formals and locals
                    Valenv finalEnv = NULL; 
                                                                       
                    // copy the formals into the final environment
                    finalEnv = mkValenv(xs, vs);
                    
                    // move the locals into the finalEnv and initialize them to 0
                    for ( tempEnv = locals; tempEnv; tempEnv = tempEnv->tl) {
                        bindval( tempEnv->hd, 0, finalEnv );
                    }

                    return eval(f.u.userdef.body, globals, functions, finalEnv);
                }
            case PRIMITIVE:
