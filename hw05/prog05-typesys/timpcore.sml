...
(* function [[ty]], to check the type of an expression, given $\itenvs$ ((prototype)) 413 *)
      ...
      | ty (AAT (a, i))        =
          let val tau_a = case ty a of
                               ARRAYTY tau => tau
                             | _           => raise TypeError "Type of a is not an array"
              val tau_i = ty i
          in
          ...
          in
            if eqType(tau_i, INTTY) then
              if eqType(tau_a, tau_e) then  
                tau_a
              else raise TypeError "Type of e is incompatible with array of type a"
            else raise TypeError "Type of i is not an integer"
          end
